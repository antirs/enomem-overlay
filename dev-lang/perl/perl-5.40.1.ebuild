...
IUSE="berkdb perl_features_debug doc gdbm perl_features_ithreads minimal perl_features_quadmath static static-libs"

LIB_DEPEND="
	berkdb? ( sys-libs/db:=[static-libs?] )
	gdbm? ( >=sys-libs/gdbm-1.8.3:=[static-libs?] )
	app-arch/bzip2[static-libs?]
	>=sys-libs/zlib-1.2.12[static-libs?]
...
src_configure() {
	...
	if use static; then
		disabled_extensions="${disabled_extensions} DynaLoader Errno NDBM_File Pod/Html Pod/Functions VMS/DCLsym VMS/Stdio"
	fi
	...
	if use static; then
		sed -i -e "s,^static_ext=.*,static_ext='B Cwd Fcntl I18N/Langinfo SDBM_File XS/APItest POSIX re XS/Typemap GDBM_File File/Glob Hash/Util'," Configure
	fi
	...
	myconf \
		$(usex static -Uuseshrplib -Duseshrplib) \
		$(usev static -Uusedl) \
		-Darchname="${myarch}" \
		-Dar="$(tc-getAR)" \
		-Dcc="$(tc-getCC)" \
	...
	if use static; then
		sed -i -e '/MANIFEST_SRT):/s,DLSUFFIX,LIB_EXT,' Makefile
	fi
}
...
src_install() {
	...
	if ! tc-is-static-only && ! use static ; then
