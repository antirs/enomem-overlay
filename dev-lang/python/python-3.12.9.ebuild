...
IUSE="
	bluetooth build debug +ensurepip examples extra gdbm libedit
...
LIB_DEPEND="
	...
	>=dev-libs/expat-2.1:=[static-libs?]
	dev-libs/libffi:=[static-libs?]
	dev-libs/mpdecimal:=[static-libs?]
	dev-python/gentoo-common
	>=sys-libs/zlib-1.1.3:=[static-libs?]
	virtual/libcrypt:=[static-libs?]
	virtual/libintl[static-libs?]
	ensurepip? ( dev-python/ensurepip-pip )
	extra? ( dev-libs/double-conversion:=[static-libs?] )
	gdbm? ( sys-libs/gdbm:=[berkdb,static-libs?] )
	kernel_linux? ( sys-apps/util-linux:=[static-libs?] )
	ncurses? ( >=sys-libs/ncurses-5.2:=[static-libs?] )
	readline? (
		!libedit? ( >=sys-libs/readline-4.1:=[static-libs?] )
		libedit? ( dev-libs/libedit:=[static-libs?] )
	)
	sqlite? ( >=dev-db/sqlite-3.3.8:3=[static-libs?] )
	ssl? ( >=dev-libs/openssl-1.1.1:=[static-libs?] )
	tk? (
		>=dev-lang/tcl-8.0:=
		>=dev-lang/tk-8.0:=
		dev-tcltk/blt:=
...
EXTRA_PATCHES=(
	"${FILESDIR}"/python-extra-modules-ujson.patch
)
STATIC_PATCHES=(
	"${FILESDIR}"/python-static-ctypes.patch
)
...
_dbm _dbmmodule.c -DUSE_BERKDB
# gdbm module needs -lgdbm
$(usev gdbm '_gdbm _gdbmmodule.c')

# needs -lreadline or -ledit, sometimes termcap, termlib, or tinfo
$(usev readline 'readline readline.c')
...
$(usev extra 'ujson ujson/python/ujson.c ujson/python/objToJSON.c ujson/python/JSONtoObj.c ujson/lib/ultrajsonenc.c ujson/lib/ultrajsondec.c ujson/lib/dconv_wrapper.cc')
...
EOF
	...
	if [[ -n "${ESYSROOT}" ]]; then
		...
		export PKG_CONFIG_LIBDIR="${ESYSROOT}"
	...
	if use extra; then
		sed -i -e '/^Modules\/ujson.*dconv.*o:/s/\$(PY_BUILTIN_MODULE_CFLAGS)/\$(subst -std=c11,-std=c++11,\$(PY_BUILTIN_MODULE_CFLAGS))/' \
			Makefile
	fi
...
src_compile() {
	...
	if use extra; then
		EXTRA_EMAKE_ARGS=(
			MODULE_UJSON_CFLAGS="-I${ESYSROOT}/usr/include/double-conversion -I./Modules/ujson/lib"
		)
		if use enomem_features_lib_libcxx; then
			EXTRA_EMAKE_ARGS+=(
				MODULE_UJSON_LDFLAGS="-ldouble-conversion -lc++"
			)
		else
			EXTRA_EMAKE_ARGS+=(
				MODULE_UJSON_LDFLAGS="-ldouble-conversion -static-libstdc++ -lstdc++"
			)
		fi
		if use static; then
			EXTRA_EMAKE_ARGS+=(
				MODULE__SQLITE3_LDFLAGS="-lsqlite3 -ldb"
			)
		fi
	fi
	...
	# in _sysconfigdata*
	emake CPPFLAGS= CFLAGS= LDFLAGS= "${EXTRA_EMAKE_ARGS[@]}"
