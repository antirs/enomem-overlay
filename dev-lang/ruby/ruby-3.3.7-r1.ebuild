...
IUSE="berkdb debug doc examples gdbm jemalloc jit noshared socks5 +ssl static static-libs systemtap tk valgrind xemacs"
...
LIB_DEPEND="
	...
	gdbm? ( sys-libs/gdbm:=[static-libs?] )
	jemalloc? ( dev-libs/jemalloc:= )
	jit? ( ${RUST_DEPEND} )
	ssl? (
		dev-libs/openssl:0=[static-libs?]
	)
	socks5? ( >=net-proxy/dante-1.1.13[static-libs?] )
...
PDEPEND="
	...
	!static? ( >=dev-ruby/bundler-2.5.11[ruby_targets_ruby33(-)] )
	!static? ( >=dev-ruby/did_you_mean-1.6.3[ruby_targets_ruby33(-)] )
	!static? ( >=dev-ruby/json-2.7.2[ruby_targets_ruby33(-)] )
	!static? ( >=dev-ruby/rdoc-6.6.2[ruby_targets_ruby33(-)] )
...
src_prepare() {
	...
	sed -i -e 's/#option nodynamic/option nodynamic/' ext/Setup || die
	sed -i -e 's/--no-extstatic/--extstatic \$(EXTSTATIC)/' template/configure-ext.mk.tmpl || die
