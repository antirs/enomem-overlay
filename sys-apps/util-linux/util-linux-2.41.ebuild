...
LIB_DEPEND="
	...
	audit? ( >=sys-process/audit-2.6:=[static-libs?] )
	caps? ( sys-libs/libcap-ng[static-libs?] )
	cramfs? ( sys-libs/zlib:=[static-libs?] )
	cryptsetup? ( >=sys-fs/cryptsetup-2.1.0[static-libs?] )
	hardlink? ( dev-libs/libpcre2:=[static-libs?] )
	ncurses? (
		sys-libs/ncurses:=[static-libs?,unicode(+)?]
		magic? ( sys-apps/file:0=[static-libs?] )
	)
	nls? ( virtual/libintl[${MULTILIB_USEDEP},static-libs?] )
	pam? ( sys-libs/pam )
	python? ( ${PYTHON_DEPS} )
	readline? ( sys-libs/readline:0=[static-libs?] )
	rtas? ( sys-libs/librtas )
	selinux? ( >=sys-libs/libselinux-2.2.2-r4[${MULTILIB_USEDEP},static-libs?] )
