...
SLOT="0"
IUSE="audit build caps +cramfs cryptsetup fdformat +hardlink kill +logger magic ncurses nls noshared pam python +readline rtas selinux slang static static-libs +su +suid systemd test tty-helpers udev unicode uuidd"

# Most lib deps here are related to programs rather than our libs,
# so we rarely need to specify ${MULTILIB_USEDEP}.
RDEPEND="
	!static? ( virtual/libcrypt:= )
	!static? ( audit? ( >=sys-process/audit-2.6:= ) )
	!static? ( caps? ( sys-libs/libcap-ng ) )
	!static? ( cramfs? ( sys-libs/zlib:= ) )
	cryptsetup? ( >=sys-fs/cryptsetup-2.1.0 )
	!static? ( hardlink? ( dev-libs/libpcre2:= ) )
	!static? ( ncurses? (
	...
	pam? ( sys-libs/pam )
	python? ( ${PYTHON_DEPS} )
	!static? ( readline? ( sys-libs/readline:0= ) )
...
DEPEND="
	...
	static? ( audit? ( >=sys-process/audit-2.6:=[static-libs] ) )
	static? ( caps? ( sys-libs/libcap-ng[static-libs] ) )
	static? ( cramfs? ( sys-libs/zlib:=[static-libs] ) )
	static? ( hardlink? ( dev-libs/libpcre2:=[static-libs] ) )
	static? ( ncurses? (
		sys-libs/ncurses:=[static-libs,unicode(+)?]
		magic? ( sys-apps/file:0=[static-libs] )
	) )
	static? ( readline? ( sys-libs/readline:0=[static-libs] ) )
	virtual/os-headers
	acct-group/root
"
RDEPEND+="
