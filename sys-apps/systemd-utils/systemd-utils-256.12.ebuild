...
IUSE="+acl boot +kmod kernel-install noshared selinux split-usr static static-libs sysusers +tmpfiles test +udev ukify"
REQUIRED_USE="
...
COMMON_DEPEND="
	elibc_musl? ( >=sys-libs/musl-1.2.3[static-libs?] )
	selinux? ( sys-libs/libselinux:0=[static-libs?] )
	tmpfiles? (
		acl? ( sys-apps/acl:0=[static-libs?] )
	)
	udev? (
		>=sys-apps/util-linux-2.30:0=[${MULTILIB_USEDEP},static-libs?]
		sys-libs/libcap:0=[${MULTILIB_USEDEP},static-libs?]
		virtual/libcrypt:=[${MULTILIB_USEDEP},static-libs?]
		acl? ( sys-apps/acl:0=[static-libs?] )
		kmod? ( >=sys-apps/kmod-15:0=[static-libs?] )
	)
	!udev? (
		>=sys-apps/util-linux-2.30:0=[static-libs?]
		sys-libs/libcap:0=[static-libs?]
...
src_prepare() {
	...
	if use static; then
		PATCHES+=(
			"${FILESDIR}"/systemd-utils-meson-static.patch
			"${FILESDIR}"/systemd-utils-strv-static.patch
			"${FILESDIR}"/systemd-utils-strutils-static.patch
		)
	fi

	sed -i -e 's,-Werror=undef,-Wno-error=undef,' meson.build

	if use static && use elibc_glibc; then
		sed -i -e 's,-Werror=format=2,-Werror=format=1,' meson.build
		sed -i -e 's,-Werror=shift-overflow=2,-Werror=shift-overflow=1,' meson.build
		sed -i -e 's/-Wl,--fatal-warnings/-Wl,--no-fatal-warnings/' meson.build
	fi
...
multilib_src_configure() {
	local default_library=$(usex static-libs both shared)
	if use static; then
