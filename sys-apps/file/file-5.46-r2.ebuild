...
LICENSE="BSD-2"
SLOT="0"
IUSE="bzip2 lzip lzma noshared python seccomp static static-libs zlib zstd"
REQUIRED_USE="python? ( ${PYTHON_REQUIRED_USE} )"

RDEPEND="
	!static? ( bzip2? ( app-arch/bzip2[${MULTILIB_USEDEP}] ) )
	!static? ( lzip? ( app-arch/lzlib ) )
	!static? ( lzma? ( app-arch/xz-utils[${MULTILIB_USEDEP}] ) )
	!static? ( seccomp? ( >=sys-libs/libseccomp-2.5.4[${MULTILIB_USEDEP}] ) )
	!static? ( zlib? ( >=sys-libs/zlib-1.2.8-r1[${MULTILIB_USEDEP}] ) )
	!static? ( zstd? ( app-arch/zstd:=[${MULTILIB_USEDEP}] ) )
	python? (
		${PYTHON_DEPS}
...
"
DEPEND="${RDEPEND}
	static? ( bzip2? ( app-arch/bzip2[static-libs,${MULTILIB_USEDEP}] ) )
	static? ( lzip? ( app-arch/lzlib[static-libs] ) )
	static? ( lzma? ( app-arch/xz-utils[static-libs,${MULTILIB_USEDEP}] ) )
	static? ( seccomp? ( >=sys-libs/libseccomp-2.5.4[static-libs,${MULTILIB_USEDEP}] ) )
	static? ( zlib? ( >=sys-libs/zlib-1.2.8-r1[static-libs,${MULTILIB_USEDEP}] ) )
	static? ( zstd? ( app-arch/zstd:=[static-libs,${MULTILIB_USEDEP}] ) )
"
BDEPEND+="
	python? (
		${PYTHON_DEPS}
...
src_compile() {
	append-ldflags -L"${ESYSROOT}/usr/$(get_libdir)" -static
	if need_build_file ; then
		# bug #586444
