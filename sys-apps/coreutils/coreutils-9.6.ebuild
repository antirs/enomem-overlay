...
LICENSE="GPL-3+"
SLOT="0"
IUSE="acl caps gmp hostname kill multicall nls noshared +openssl selinux +split-usr static static-libs test vanilla xattr"
RESTRICT="!test? ( test )"
...
src_configure() {
	...
	if use static; then
		sed -i -e '/\$(src_libstdbuf_so_LDFLAGS) \$(LDFLAGS)/s/\$(src_libstdbuf_so_LDFLAGS) \$(LDFLAGS)/$(src_libstdbuf_so_LDFLAGS) $(subst -static,,$(LDFLAGS))/' Makefile
	fi
}
...
src_install() {
	...
	if use noshared ; then
		find "${ED}"/usr/libexec -type f -name '*.so' -delete
	fi
}
