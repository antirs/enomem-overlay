...
EAPI=8
inherit flag-o-matic libtool multilib-minimal toolchain-funcs verify-sig
...
IUSE="
	acl blake2 +bzip2 +e2fsprogs expat +iconv lz4 +lzma lzo nettle noshared
	static static-libs test xattr +zstd
"
RESTRICT="!test? ( test )"

RDEPEND="
	sys-libs/zlib:=[static-libs?,${MULTILIB_USEDEP}]
	acl? ( virtual/acl:=[static-libs?,${MULTILIB_USEDEP}] )
	blake2? ( app-crypt/libb2:=[static-libs?,${MULTILIB_USEDEP}] )
	bzip2? ( app-arch/bzip2:=[static-libs?,${MULTILIB_USEDEP}] )
	expat? ( dev-libs/expat:=[static-libs?,${MULTILIB_USEDEP}] )
	!expat? ( dev-libs/libxml2:=[static-libs?,${MULTILIB_USEDEP}] )
	iconv? ( virtual/libiconv:=[${MULTILIB_USEDEP}] )
	dev-libs/openssl:=[${MULTILIB_USEDEP}]
	lz4? ( >=app-arch/lz4-0_p131:=[static-libs?,${MULTILIB_USEDEP}] )
	lzma? ( >=app-arch/xz-utils-5.2.5-r1:=[static-libs(+)?,${MULTILIB_USEDEP}] )
	lzo? ( >=dev-libs/lzo-2:=[static-libs?,${MULTILIB_USEDEP}] )
	nettle? ( dev-libs/nettle:=[static-libs?,${MULTILIB_USEDEP}] )
...
multilib_src_configure() {
	...
	if multilib_is_native_abi ; then
		myconf+=(
			--enable-bsdcat="$((tc-is-static-only || use static) && echo static || echo shared)"
			--enable-bsdcpio="$((tc-is-static-only || use static) && echo static || echo shared)"
			--enable-bsdtar="$((tc-is-static-only || use static) && echo static || echo shared)"
			--enable-bsdunzip="$((tc-is-static-only || use static) && echo static || echo shared)"
		)
	else
		myconf+=(
			--disable-bsdcat
	...
	if use static; then
		local _ldflags="-static-libstdc++ -lstdc++"
