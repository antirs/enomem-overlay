...
LICENSE="BSD"
SLOT="0"
IUSE="${CMAKE_DOCS_USEFLAG} dap gui ncurses static static-libs test"
RESTRICT="!test? ( test )"
REQUIRED_USE="
	static? ( !dap )
	static? ( !gui )
...
LIB_DEPEND="
	>=app-arch/libarchive-3.3.3:=[static-libs?]
	app-crypt/rhash:0=[static-libs?]
	>=dev-libs/expat-2.0.1[static-libs?]
	>=dev-libs/jsoncpp-1.9.2-r2:0=[static-libs?]
	>=dev-libs/libuv-1.10.0:=[static-libs?]
	sys-libs/zlib[static-libs?]
	virtual/pkgconfig
	dap? ( dev-cpp/cppdap )
	gui? ( dev-qt/qtbase:6[gui,widgets] )
	ncurses? ( sys-libs/ncurses:=[static-libs?] )
	>=net-misc/curl-7.21.5[static-libs?,ssl]
"
RDEPEND="
	!static? ( ${LIB_DEPEND} )
"
...
src_configure() {
	local mycmakeargs=(
		...
		$(usev static -DBUILD_SHARED_LIBS=OFF)
	)
	...
	if use static; then
		local _link_static="-ldl -lcurl -lzstd -lzstd -pthread -lgnutls -lgmp -lunistring -latomic -lhogweed -lgmp -ltasn1 -lidn2 -lgnutls -lgmp -lunistring -latomic -lhogweed -lgmp -lnettle -ltasn1 -lidn2 -lunistring -lz -larchive -lacl -llzo2 -lzstd -llz4 -lbz2 -lz -llzma -lexpat -lxml2 -lcrypto -ldl -pthread -Wl,--push-state,--as-needed,-latomic,--pop-state -lxml2 -lm -lz -licui18n -licuuc -licudata -lpthread -lm -ljsoncpp -luv -lrhash ../Utilities/cmllpkgc/libcmllpkgc.a"
		if use enomem_features_lib_libcxx && use elibc_musl; then
			_link_static="-ldl -lcurl -lzstd -lzstd -pthread -lgnutls -lgmp -lunistring -lhogweed -lgmp -ltasn1 -lidn2 -lgnutls -lgmp -lunistring -lhogweed -lgmp -lnettle -ltasn1 -lidn2 -lunistring -lz -larchive -lacl -llzo2 -lzstd -llz4 -lbz2 -lz -llzma -lexpat -lxml2 -lcrypto -ldl -pthread -lxml2 -lm -lz -licui18n -licuuc -licudata -lpthread -lm -ljsoncpp -luv -lrhash ../Utilities/cmllpkgc/libcmllpkgc.a"
		fi
		sed -i -e "s%-ldl .*%${_link_static}%" "${BUILD_DIR}"/Source/CMakeFiles/cmake.dir/link.txt || die
		sed -i -e "s%-ldl .*%${_link_static}%" "${BUILD_DIR}"/Source/CMakeFiles/cpack.dir/link.txt || die
		sed -i -e "s%-ldl .*%${_link_static}%" "${BUILD_DIR}"/Source/CMakeFiles/ctest.dir/link.txt || die
	fi
}
