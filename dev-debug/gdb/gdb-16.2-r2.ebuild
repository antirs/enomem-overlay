...
SLOT="0"
IUSE="babeltrace cet debuginfod guile lzma multitarget nls +python rocm +server sim source-highlight static static-libs test vanilla xml xxhash zstd"
...
LIB_DEPEND="
	...
	>=sys-libs/ncurses-5.2-r2:=[static-libs?]
	>=sys-libs/readline-7:=[static-libs?]
	sys-libs/zlib[static-libs?]
	babeltrace? ( dev-util/babeltrace:= )
	debuginfod? (
		dev-libs/elfutils[debuginfod(-),static-libs?]
	)
	lzma? ( app-arch/xz-utils[static-libs?] )
	python? ( ${PYTHON_DEPS} )
	guile? ( ${GUILE_DEPS} )
	xml? ( dev-libs/expat[static-libs?] )
...
src_prepare() {
	...
	if [[ -n "${ESYSROOT}" ]] && use static; then
		sed -i -e "s,gmplibs=\"-lmpfr -lgmp\",gmplibs=\"-L${ESYSROOT}/usr/$(get_libdir) -l:libmpfr.a -lgmp\",g" \
			configure{.ac,} || die
	fi
...
src_configure() {
	...
	if [[ -n "${ESYSROOT}" ]]; then
		...
		append-cflags -I"${ESYSROOT}"/usr/include
	...
	use static && append-cflags -Wno-error=implicit-function-declaration
	...
	if [[ -n "${ESYSROOT}" ]]; then
		sed -i -e '/\$(COMPILE.c).*\$(srcdir)\/obstack.c/s%\$(COMPILE.c)%$(subst '"-I${ESYSROOT}/usr/include"',,$(COMPILE.c))%' libiberty/Makefile.in || die
	fi
}
