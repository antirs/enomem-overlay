...
SLOT="3"
IUSE="debug doc icu noshared +readline secure-delete static static-libs tcl test tools"
RESTRICT="!test? ( test )"

LIB_DEPEND="
	sys-libs/zlib:=[${MULTILIB_USEDEP},static-libs?]
	icu? ( dev-libs/icu:=[${MULTILIB_USEDEP},static-libs?] )
	readline? ( sys-libs/readline:=[${MULTILIB_USEDEP},static-libs?] )
...
multilib_src_configure() {
	...
	if use static; then
		CC_FOR_BUILD=${BUILD_CC} \
					LDFLAGS=$(usev readline '-lreadline -lncurses -ltinfo')
					econf "${options[@]}"
	else
...
multilib_src_compile() {
	emake CFLAGS=$(usev static -static) HAVE_TCL="$(usev tcl 1)" TCLLIBDIR="${EPREFIX}/usr/$(get_libdir)/${P}"
