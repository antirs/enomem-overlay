...
LICENSE="GPL-2"
SLOT="0"
IUSE="+curl cgi cvs doc keyring +gpg highlight +iconv mediawiki +nls +pcre perforce +perl +safe-directory selinux static static-libs subversion test tk +webdav xinetd"
...
LIB_DEPEND="
	...
	iconv? ( virtual/libiconv[static-libs?] )
...
STATIC_PATCHES=(
	"${FILESDIR}"/git-meson-static-dependencies.patch
)
...
src_configure() {
	...
	local default_library=$(usex static both shared)
	if use static; then
