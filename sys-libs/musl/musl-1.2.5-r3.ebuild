...
SLOT="0"
IUSE="crypt headers-only split-usr static"
...
MIRAGE_PATCHES=(
	"${FILESDIR}"/0001-avoid-inline-functions-involving-long-double-in-math.patch
	"${FILESDIR}"/0002-use-preferred-syntactic-location-for-epoll_event-pac.patch
)

if [[ "${ENOMEM_FEATURES}" =~ "toolchain_cproc" ]]; then
	PATCHES+=("${MIRAGE_PATCHES[@]}")
fi
...
src_compile() {
	...
	if ! is_crosspkg ; then
		emake -C "${T}" getconf getent iconv \
			...
			CFLAGS="${CFLAGS}" \
			CPPFLAGS="${CPPFLAGS}" \
			LDFLAGS="${LDFLAGS} $(usev static '-static')" \
			VPATH="${WORKDIR}/misc"
	fi
