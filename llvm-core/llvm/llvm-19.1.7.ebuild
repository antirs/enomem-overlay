...
inherit cmake flag-o-matic llvm.org multilib-minimal pax-utils preserve-libs
...
IUSE="
	+binutils-plugin bootstrap-gcc debug debuginfod doc exegesis libedit +libffi
	test xml z3 zstd
"
RESTRICT="!test? ( test )"
...
pkg_preinst() {
	# preserve libstdc++ LLVM to rebuild libc++ clang with stdlibc++ clang
	if [[ $(tc-get-cxx-stdlib) == libc++ ]]; then
		preserve_old_lib \
			"/usr/lib/llvm/${LLVM_MAJOR}/$(get_libdir)/libLLVM.so.19.1"
	fi
}
