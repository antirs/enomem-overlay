...
IUSE="cxx +large noshared static-libs +threads"

RDEPEND="
	!static-libs? ( >=dev-libs/libatomic_ops-7.4[${MULTILIB_USEDEP}] )
"
DEPEND="${RDEPEND}
	static-libs? ( >=dev-libs/libatomic_ops-7.4[static-libs,${MULTILIB_USEDEP}] )
"
...
multilib_src_configure() {
	...
	if [[ -n "${ESYSROOT}" ]]; then
		...
		CFLAGS="${CFLAGS} -I${ESYSROOT}/usr/include" ECONF_SOURCE="${S}" \
			  econf "${config[@]}"
	else
