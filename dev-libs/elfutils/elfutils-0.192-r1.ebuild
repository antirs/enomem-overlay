...
SLOT="0"
IUSE="bzip2 debuginfod lzma nls noshared static static-libs stacktrace test +utils valgrind zstd"
RESTRICT="!test? ( test )"
...
multilib_src_configure() {
	...
	if [[ -n "${ESYSROOT}" ]]; then
		...
		if use enomem_features_lib_libcxx; then
			append-cflags -I"${ESYSROOT}"/usr/include/c++/v1
			append-cxxflags -I"${ESYSROOT}"/usr/include/c++/v1
		fi
		append-cflags -I"${ESYSROOT}"/usr/include
		append-cxxflags -I"${ESYSROOT}"/usr/include
