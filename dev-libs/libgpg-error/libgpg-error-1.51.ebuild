...
IUSE="common-lisp nls noshared static static-libs test"
RESTRICT="!test? ( test )"

LIB_DEPEND="
	nls? ( >=virtual/libintl-0-r1[${MULTILIB_USEDEP},static-libs?] )
"
RDEPEND="
	!static? ( ${LIB_DEPEND} )
"
...
multilib_src_configure() {
	...
	if use static; then
		PKG_CONFIG="$(tc-getPKG_CONFIG) --static" ECONF_SOURCE="${S}" econf "${myeconfargs[@]}"
	else
