...
IUSE="afs bashlogger examples mem-scramble +net nls plugins pgo +readline static static-libs"

LIB_DEPEND="
	>=sys-libs/ncurses-5.2-r2:=[static-libs?]
	nls? ( virtual/libintl[static-libs?] )
	static? ( >=sys-libs/ncurses-5.2-r2:=[tinfo] )
"
RDEPEND="
	!static? ( ${LIB_DEPEND} )
"
...
"
if (( PLEVEL >= 0 )); then
	RDEPEND+=" !static? ( readline? ( >=sys-libs/readline-${READLINE_VER}:=[static-libs?] ) )"
	DEPEND+=" readline? ( >=sys-libs/readline-${READLINE_VER}:=[static-libs?] )"
fi
